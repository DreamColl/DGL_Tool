...
$(function () {
    ...
    function createSchool() {
        ...
        for (var i = school_begin[area]; i < school_end[area]; i++) {
            $("select[name=school]").append("<option value=" + i + ">" + SCHOOL[i] + "</option>");
        }
    }
    ...
    function createScore() {
        ...
        for (var i = 0; i < times; i++) {
            $('div#school').append("<ul class='school'><a>第" + NumberToChinese(i + 1) + "场:</a><br><select name='score_a" + i + "'></select><select name='score_b" + i + "'></select><p id='bestName-all'><a id='bestName'> 最佳辩手:</a><input type='text' name='bestName'></p></ul>");
        }
...
function setData() {
    //收集数据
    var area = $("select[name = area]>option:selected");
    var year = $("select[name = year]>option:selected");
    var turn = $("select[name = turn]>option:selected");

    var school = $("select[name=school]>option:selected");
    var score = $("select[name^=score]>option:selected");

    //shcool:school,score:score
    var dataValue = 0;
    var dataArr = [];
    var s = new Set();
    for (var i = 0; i < school.length; i++) {
        var id = year.val() + area.val() + turn.val() + school[i].value;
        var data = {
            area: area.val(),
            year: year.val(),
            turn: turn.val(),
        ...
        dataArr.push(data);
        s.add(school[i].value);
        dataValue += parseInt(data.score);
    }
    ...
    // if (dataValue == school.length * 9 / 2 || Object.keys(data).length == school.length + 3 ||s.size == school.length)
    //     return data;
    // else
